...
export const AboutGhazi = () => {
  ...
  const { c } = usePageContent('about-ghazi');
  ...
  const cats = [1, 2, 3, 4].map((n) => ({ title: c(`sol.c${n}title`), img: c(`sol.c${n}img`) }));
  const focusIcon = (n) => {
    if (n === 1) return <Award size={20} />;
    if (n === 2) return <Shield size={20} />;
    if (n === 3) return <Layers size={20} />;
          ...
          <div className="dual-brand-grid">
            <div>
              ...
              <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                {toList(c('who.tags')).map((tag, i) => (
                  <span key={i} className="badge-tag" style={{ background: 'var(--green-600)', color: '#FFF' }}>{tag}</span>
                ))}
              </div>
            </div>

            <div style={{ position: 'relative', borderRadius: '24px', overflow: 'hidden', height: '360px', boxShadow: 'var(--shadow-md)' }}>
              <img src={slides[activeSlide]} alt="Ghazi" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              <button onClick={prev} style={{ position: 'absolute', left: '16px', top: '50%', transform: 'translateY(-50%)', width: '40px', height: '40px', borderRadius: '50%', background: 'rgba(255,255,255,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--black)' }} aria-label="Previous">
                <ChevronLeft size={20} />
              </button>
              <button onClick={next} style={{ position: 'absolute', right: '16px', top: '50%', transform: 'translateY(-50%)', width: '40px', height: '40px', borderRadius: '50%', background: 'rgba(255,255,255,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--black)' }} aria-label="Next">
        ...
        <div className="container">
          ...
          <div className="vision-mission-grid">
            <div style={{ position: 'relative', borderRadius: '24px', overflow: 'hidden', boxShadow: 'var(--shadow-md)' }}>
              <img src={c('vm.image')} alt="Ghazi" style={{ width: '100%', height: '340px', objectFit: 'cover' }} />
