...
export const AdminTestimoni = () => {
  ...
  return (
    <div>
      ...
      {/* Testimonials List */}
      {loading ? (
        <div style={{ padding: '60px', textAlign: 'center', color: '#667085' }}>Memuat testimoni...</div>
      ) : items.length === 0 ? (
            ...
            >
              <div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '14px' }}>
                  ...
                  />
                  <div style={{ flex: 1 }}>
                    <h3 style={{ fontSize: '0.95rem', fontWeight: 700, color: '#121212', margin: 0 }}>
                      {item.name}
                    </h3>
                    <div style={{ fontSize: '0.75rem', color: '#667085' }}>
                      {item.time_text || item.time || 'Pelanggan'} • <span style={{ color: 'var(--green-700)', fontWeight: 600 }}>{item.category}</span>
                    </div>
                  </div>
                </div>
                ...
                {/* Review Content */}
                <p style={{ fontSize: '0.875rem', color: '#344054', lineHeight: 1.6, fontStyle: 'italic', marginBottom: '16px' }}>
      ...
      {isModalOpen && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50, padding: '20px' }}>
          <div style={{ background: '#FFFFFF', borderRadius: '20px', width: '100%', maxWidth: '500px', padding: '28px', boxShadow: '0 20px 40px rgba(0,0,0,0.2)' }}>
